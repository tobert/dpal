...
- **Working notes (three-file pattern).** `signoff.md` (repo root, ephemeral, **not committed**) is the cross-session handoff a fresh process can't reconstruct — what we were doing and what's next; melt it into the durable docs and delete it once it has a home. **`docs/issues.md`** is the live work tracker — skim it before proposing new work; **delete entries when they ship** rather than marking them done. `docs/devlog.md` is the landed-work narrative (the *why* behind a ship; dated, newest-first) — `git log` is canonical, this is the story. Code is truth; the notes track what isn't in it yet.
